...
namespace Tetris
{
    class Game
    {
        ...
        //游戏结束返回ture
        public bool EndCheck(Pattern p, Cube[,] canvus)
        {
        ...
        //炸弹
        public void Bombard(Pattern p, Cube[,] canvus)
        {
            int xx, yy;
            for (int x = -1; x < 2; x++)
            {
        ...
        //挑战模式用
        public Pattern RandomPattern2()
        {
            int p = new Random(Guid.NewGuid().GetHashCode()).Next(0, 10000) % 16;
            //int p = 15;
            //为调整概率
            if (p == 1 || p == 8)
            {
